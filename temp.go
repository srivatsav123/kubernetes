...
type KernelHandler interface {
	...
	CheckBuiltinModules(filter []string) (bool, error)
}
...
// GetModules returns all installed kernel modules.
func (handle *LinuxKernelHandler) CheckBuiltinModules(filter []string) (bool, error) {
	knernelVersion, err := handle.executor.Command("uname", "-r").CombinedOutput()
	if err != nil {
		return false, err
	}
	path := fmt.Sprintf("/lib/modules/%s/modules.builtin", knernelVersion)
	for _, filterStr := range filter {
		out, err := handle.executor.Command("grep", filterStr, path).CombinedOutput()
		if err != nil {
			return false, err
		}
		if out == nil {
...
func CanUseIPVSProxier(handle KernelHandler, ipsetver IPSetVersioner) (bool, error) {
	filterStr := []string{"ip_vs ", "ip_vs_rr", "ip_vs_wrr", "ip_vs_sh", "nf_conntrack_ipv4"}
	hasBuiltinMods, err := handle.CheckBuiltinModules(filterStr)
	if hasBuiltinMods {
		return true, nil
	}
